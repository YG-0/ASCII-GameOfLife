...
options =
  ...
  , Option ['s'] ["steps"] (ReqArg (Steps . read) "NUM") "Number of steps"
  , Option ['d']
           ["delay"]
           (ReqArg (Delay . read) "MS")
...
parseOpts
  :: [Flag]
  ->          -- list of command line arguments flags
     String
...
playNSteps n delay board = do
  ...
  if n == 0 then return () else playNSteps (n - 1) delay $ boardNextState board
