...
showBoard board =
    let ((x0,y0), (xf,yf)) = bounds board 
        rowToStr x = foldl' (\acc y -> acc ++ show (board!(x,y))) "" [y0..yf]
...
readBoard str =
    if not $ isValidBoardLines boardLines
    then Nothing
    else Just $ listArray ((0, 0), (rows - 1, cols - 1)) $ concat justBoardLines
        where boardLines = map readBoardLine $ lines str
...
boardNextState board =
    ...
    in listArray bnd [cellNextState (board!i) (aliveNeigbhoursBoard!i) | i <- ind]
...
aliveNeighbours board (x, y) =
    ...
    in length $ filter isAlive $ map (\(i, j) -> cell i j) $ neighboursIndexes (x, y)
